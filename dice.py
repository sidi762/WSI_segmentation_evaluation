    ...
    for root, dirs, files in os.walk(maskPath):
        for file in files:
            ...
            if os.path.exists(resultFilePath):
                results.append(pool.apply_async(calculateDICE, (resultFilePath, maskFilePath)))

    pool.close()
    pool.join()
    resultOutput = open((resultPrefix+'dice.txt'), mode='w+',encoding='utf-8')
    total = 0
    numOfResults = 0
    for res in results:
